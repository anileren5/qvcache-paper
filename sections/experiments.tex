...
QVCache is configured with $\alpha = 0.9$, $n_{\text{buckets}} = 8$, and $d_{\text{reduced}} = 16$. We set $D$ to 0.25 for SIFT and SpaceV, and to 0.075 for the remaining datasets. The cache capacity is fixed at 100{,}000 vectors and partitioned into $n_{\text{mini-index}} = 4$ mini-indexes, each with a capacity of 25{,}000. The \texttt{ADAPTIVE} search strategy is applied to all datasets, except GloVe, which uses the \texttt{EXHAUSTIVE} strategy.
...
Queries that result in a cache hit have sub-millisecond latencies, typically between 0.1 and 1~ms, with the majority clustered around 0.1–0.2~ms. We see that hit latencies are around 1ms for GloVe as it uses \texttt{EXHAUSTIVE} and scan all mini-indexes all the time. These high hit ratios, combined with sub-millisecond hit latencies, yield up to 60–300$\times$ lower p50 latency compared to using DiskANN only without QVCache.
...
QVCache adapts to varying $k$ values, as shown in Figure~\ref{fig:k-experiments}. Since the working set size grows linearly with $k$, we increased QVCache’s capacity for $k = 100$ linearly. Across all $k$ values, QVCache preserves high recall and hit ratio, while latency reductions become more pronounced with larger $k$, reaching up to 950$\times$ lower for $k = 100$.
