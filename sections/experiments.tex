...
QVCache is configured with $\alpha = 0.9$, $n_{\text{buckets}} = 8$, and $d_{\text{reduced}} = 16$. We set $D$ to 0.25 for SIFT and SpaceV, and to 0.075 for the remaining datasets. The cache capacity is fixed at 100{,}000 vectors and partitioned into $n_{\text{mini-index}} = 4$ mini-indexes, each with a capacity of 25{,}000. The \texttt{ADAPTIVE} search strategy is applied to all datasets, except GloVe, which uses the \texttt{SEQUENTIAL} strategy.
...
Queries that result in a cache hit have sub-millisecond latencies, typically between 0.1 and 1~ms, with the majority clustered around 0.1–0.2~ms. We see that hit latencies are around 1ms for GloVe as it uses \texttt{SEQUENTIAL} and scan all mini-indexes all the time. These high hit ratios, combined with sub-millisecond hit latencies, yield up to 60–300× lower p50 latency.

Despite these significant performance gains, recall is only slightly impacted, dropping by 2–5\%. This impact can be further mitigated by tuning $D$, at the cost of some reduction in hit rate, as discussed in Section~\ref{sec:deviation-factor}.
...
QVCache adapts to varying $k$ values, as shown in Figure~\ref{fig:k-experiments}. Since the working set size grows linearly with $k$, we increased QVCache’s capacity for $k = 100$ linearly. Across all $k$ values, QVCache maintains recall ($k\text{-recall}@k$) and hit ratio, while latency improvements become more pronounced with larger $k$, reaching up to 950× for $k = 100$.
...
In summary, we recommend setting the total cache capacity $N$ (in vectors) large enough to accommodate the expected working set size, as is standard practice for any caching system, and generally larger. Thanks to the access skew described in Section~\ref{sec:workload-characteristics}, this is relatively inexpensive to provision. The working set size grows linearly with $k$ and with the number of diverse (semantically dissimilar) queries arriving within a caching interval (i.e., queries per window in Figure~\ref{fig:sliding-window}). A best-effort estimate of the working set can guide the choice of $c_{\text{mini-index}}$, as suggested in Section~\ref{sec:granularity-matters}. Even if this estimate is imperfect, recall and hit ratio remain largely unaffected (Figure~\ref{fig:cache_size_experiments}b), though hit latencies may increase.
