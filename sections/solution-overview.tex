...
For workloads that require insertions and deletions of vectors (assuming the index on backend database supports these operations), QVCache does not yet handle them natively. Insert and delete queries should therefore be routed directly to the backend, bypassing QVCache. The resulting updates eventually propagate into QVCache through evictions and subsequent cache fills, and the approximate nature of vector search makes this eventual consistency acceptable in practice. As future work, we plan to add native support for insertions and deletions by buffering them in QVCache and asynchronously applying them to the backend, which we expect to further improve insertion and deletion throughput.