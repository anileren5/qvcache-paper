...
\begin{figure}[!htb]
  ...
  \caption{Handling cache hits and misses in QVCache with four mini-indexes, each with a capacity of three vectors. Mini-indexes are ordered by the eviction policy metadata: the leftmost is the hottest (MRU), and the rightmost is the coldest (LRU).}
...
In case of insertions and deletions of raw vectors other than the search, QVCache does not handle them directly but route them to the backend database. The resulting updates eventually propagate into QVCache through subsequent cache fills and evictions. %, and the approximate nature of vector search makes this eventual consistency acceptable in practice. 
