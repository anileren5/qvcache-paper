...
QVCache is designed as a drop-in layer for existing ANN systems. It requires no changes to index construction, applies uniformly to any backend vector database, independent of whether it is in-memory or disk-based. %Unlike ANN indexes, QVCache does not grow with dataset size, does not aim for asymptotic recall completeness, and falls back to backend search on cache misses. 
Leveraging semantic locality, QVCache converts costly backend queries into cache hits, reducing both backend load and query latency with minimal memory overhead. This enables meeting latency SLOs at high recall without requiring in-memory indexes to scale with the dataset size; instead, they scale with the \emph{working set}, which is much smaller in practice \cite{incrementalivfindexmaintenance}.
